...
import { Room, Design, MaterialBreakdownItem, ChatMessage, SupplierRequest, FloorplanAnalysis, ProjectPayload } from '../types';
...
interface DesignStudioProps {
  ...
  analysisResult: FloorplanAnalysis;
}
...
const DesignStudio: React.FC<DesignStudioProps> = ({ room, onBack, authToken, floorplanImage, analysisResult }) => {
  ...
  const handleSaveProject = async () => {
      if (!projectName.trim() || designs.length === 0) return;
      setIsSaving(true);
      setError(null);
      try {
          const designPayload = designs.map(d => ({
              title: d.title,
              image_data: d.imageUrl.split(',')[1], // Send only base64 data
          ...
          await saveProject(payload, authToken);
          setSaveSuccess(true);
          setTimeout(() => {
  ...
  return (
    <div>
        <div className="flex justify-between items-center mb-6">
            <button onClick={onBack} className="flex items-center text-sm text-[#aaaaaa] hover:text-[#D4AF37] transition-colors">
            ...
            <button
                onClick={() => {
                    setError(null);
                    setSaveSuccess(false);
                    setProjectName(room.name.replace(/_/g, ' ') + ' Design');
                    setShowSaveModal(true);
                }}
                disabled={designs.length === 0 || saveSuccess}
                className="px-6 py-2 text-sm font-medium rounded-full border-2 border-[#D4AF37] text-[#D4AF37] bg-[rgba(212,175,55,0.1)] hover:bg-[rgba(212,175,55,0.2)] hover:shadow-[0_5px_20px_rgba(212,175,55,0.3)] transition-all duration-300 transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
              >
                {saveSuccess ? 'Project Saved!' : 'Save Project'}
              </button>
        </div>
        ...
        {showSaveModal && (
            <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-20" onClick={() => setShowSaveModal(false)}>
                <div className="bg-[#0a0a0a] border border-[rgba(212,175,55,0.25)] rounded-lg shadow-xl p-8 max-w-lg w-full" onClick={e => e.stopPropagation()}>
                    <h3 className="text-xl font-bold font-display gold-gradient-text mb-4">Save Project</h3>
                    {saveSuccess ? (
                        <div className="text-center py-4">
                            <p className="text-lg text-green-400">Project Saved Successfully!</p>
                            <p className="text-sm text-text-muted mt-2">You can now close this window and view it on your dashboard.</p>
                        </div>
                    ) : (
                    <>
                        <p className="text-sm text-[#aaaaaa] mb-4">Give your project a name to save it to your dashboard.</p>
                        <div>
                          <label htmlFor="project-name" className="block text-sm font-medium text-text-muted">Project Name</label>
                          <input
                            type="text"
                            id="project-name"
                            value={projectName}
                            onChange={(e) => setProjectName(e.target.value)}
                            className="mt-1 block w-full rounded-md bg-black/30 border-[rgba(212,175,55,0.25)] shadow-sm py-2 px-3 focus:border-[#D4AF37] focus:ring-[#D4AF37]"
                          />
                        </div>
                        {error && <p className="text-red-400 text-sm mt-2">{error}</p>}
                        <div className="flex justify-end gap-4 mt-6">
                            <button onClick={() => setShowSaveModal(false)} className="px-6 py-2 text-sm font-medium rounded-full text-text-muted hover:bg-card-bg">Cancel</button>
                            <button onClick={handleSaveProject} disabled={isSaving || !projectName} className="px-6 py-2 text-sm font-medium rounded-full bg-primary-gold text-black hover:opacity-90 disabled:opacity-50">
