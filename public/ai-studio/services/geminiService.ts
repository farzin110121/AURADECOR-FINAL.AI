...
const getAiClient = () => {
    // This is now populated by a script in index.html that fetches the key from a proxy
    const API_KEY = (window as any).process?.env?.API_KEY;

    if (!API_KEY) {
        throw new Error("API_KEY is not available. It should be loaded and set on the window object.");
    }
